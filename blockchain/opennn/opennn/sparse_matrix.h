...
public:
    ...
    bool compare_rows(const size_t&, const SparseMatrix<T>&, const size_t&) const;
    ...
    const Vector<size_t>& get_rows_indices(void) const;

    const Vector<size_t>& get_columns_indices(void) const;

    const Vector<T>& get_matrix_values(void) const;
    ...
    SparseMatrix<T> remove_row(const size_t&) const;
    SparseMatrix<T> remove_rows(const Vector<size_t>&) const;

    SparseMatrix<T> remove_rows_with_value(const T&) const;

    SparseMatrix<T> remove_first_rows(const size_t&) const;
    SparseMatrix<T> arrange_first_rows(const size_t&) const;

    SparseMatrix<T> remove_last_rows(const size_t&) const;
    SparseMatrix<T> arrange_last_rows(const size_t&) const;

    SparseMatrix<T> remove_column(const size_t&) const;
    SparseMatrix<T> remove_columns(const Vector<size_t>&) const;
    ...
    SparseMatrix<double> calculate_covariance_sparse_matrix(void) const;
    ...
    SparseMatrix<T> calculate_absolute_value(void) const;

    SparseMatrix<T> calculate_transpose(void) const;

    T calculate_determinant(void) const;

    SparseMatrix<T> calculate_cofactor(void) const;

    SparseMatrix<T> calculate_inverse(void) const;

    SparseMatrix<T> calculate_LU_inverse(void) const;
    ...
    SparseMatrix<T> calculate_eigenvalues(void) const;
    SparseMatrix<T> calculate_eigenvectors(void) const;
    ...
    SparseMatrix<size_t> to_size_t_SparseMatrix(void) const;
    SparseMatrix<double> to_double_SparseMatrix(void) const;
    SparseMatrix<string> to_string_SparseMatrix(const size_t& = 3) const;

    Matrix<T> to_matrix(void) const;
...
inline const T SparseMatrix<T>::operator () (const size_t& row, const size_t& column) const
{
    ...
    if(rows_indices.contains(row) && columns_indices.contains(column))
    {
        ...
        for(size_t i = 0; rows_indices_size; i++)
        {
...
bool SparseMatrix<T>::operator == (const SparseMatrix<T>& other_sparse_matrix) const
{
    ...
    else if(other_nonzero_columns_number != 0 && other_nonzero_rows_number != 0)
    {
        for(size_t i = 0; i < other_nonzero_rows_number; i++)
        {
            ...
            const Vector<size_t> this_equal_rows_indices = rows_indices.arrange_equal_to_indices(current_row_index);
            const Vector<size_t> this_equal_columns_indices = columns_indices.arrange_equal_to_indices(current_column_index);
...
bool SparseMatrix<T>::operator == (const Matrix<T>& other_matrix) const
{
    ...
    {
        if(!rows_indices.contains(i) && other_matrix.arrange_row(i).count_equal_to(T()) == other_columns_number)
        {
            continue;
        }
...
bool SparseMatrix<T>::operator > (const SparseMatrix<T>& other_sparse_matrix) const
{
    ...
    {
        for(size_t j = 0; j < columns_number; i++)
        {
...
template <class T>
const size_t& SparseMatrix<T>::get_rows_number(void) const
{
    return(rows_number);
}
...
template <class T>
const size_t& SparseMatrix<T>::get_columns_number(void) const
{
    return(columns_number);
}

// Vector<size_t> get_rows_indices(void) const method

/// Returns the indices of rows in the sparse matrix.

template <class T>
const Vector<size_t>& SparseMatrix<T>::get_rows_indices(void) const
{
    return(rows_indices);
}

// Vector<size_t> get_columns_indices(void) const method

/// Returns the indices of columns in the sparse matrix.

template <class T>
const Vector<size_t>& SparseMatrix<T>::get_columns_indices(void) const
{
    return(columns_indices);
}

// size_t get_columns_indices(void) const method

/// Returns the values in the sparse matrix.

template <class T>
const Vector<T>& SparseMatrix<T>::get_matrix_values(void) const
{
...
{
    Vector<size_t> greater_than_indices = rows_indices.calculate_greater_equal_to_indices(new_rows_number);

    if(greater_than_indices.size() > 0)
    {
        rows_indices = rows_indices.remove_elements(greater_than_indices);
        columns_indices = columns_indices.remove_elements(greater_than_indices);
        matrix_values = matrix_values.remove_elements(greater_than_indices);
    }
...
void SparseMatrix<T>::set_element(const size_t& row_index, const size_t& column_index, const T& value)
{
    ...
    Vector<size_t> equal_row_indices = rows_indices.arrange_equal_to_indices(row_index);
    Vector<size_t> equal_columns_indices = columns_indices.arrange_equal_to_indices(column_index);
...
template <class T>
size_t SparseMatrix<T>::count_diagonal_elements(void) const
{
...
template <class T>
size_t SparseMatrix<T>::count_off_diagonal_elements(void) const
{
    const size_t elements_number = matrix_values.size();
...
size_t SparseMatrix<T>::count_equal_to(const size_t& column_index, const T& value) const
{
    ...
    const Vector<size_t> column_nonzero_indices = columns_indices.arrange_equal_to_indices(column_index);
    ...
    {
        count = matrix_values.arrange_subvector(column_nonzero_indices).count_equal_to(value);
    }
...
size_t SparseMatrix<T>::count_not_equal_to(const size_t& column_index, const T& value) const
{
    ...
    {
        count = rows_number - matrix_values.arrange_subvector(column_nonzero_indices).count_equal_to(value);
    }
...
template <class T>
Vector<T> SparseMatrix<T>::arrange_row(const size_t& i) const
{
    // Control sentence (if debug)
...
template <class T>
Vector<T> SparseMatrix<T>::arrange_rows(const size_t& first_index, const size_t& last_index) const
{
#ifdef __OPENNN_DEBUG__
...
template <class T>
Vector<T> SparseMatrix<T>::arrange_row(const size_t& row_index, const Vector<size_t>& column_indices) const
{
    // Control sentence (if debug)
    ...
    if(!rows_indices.contains(row_index) || !columns_indices.contains(column_indices))
    {
        return row;
    }

    const Vector<size_t> equal_to_row_indices = rows_indices.arrange_equal_to_indices(row_index);

    const Vector<size_t> equal_to_column_indices = columns_indices.arrange_equal_to_indices(column_indices);

    const Vector<size_t> intersection = equal_to_row_indices.get_intersection(equal_to_column_indices);

    for(size_t j = 0; j < intersection.size(); j++)
    {
        const size_t current_row_index = columns_indices[intersection[j]];

        row[current_row_index] = matrix_values[intersection[j]];
    }
...
template <class T>
Vector<T> SparseMatrix<T>::arrange_column(const size_t& j) const
{
    // Control sentence (if debug)
...
template <class T>
Vector<T> SparseMatrix<T>::get_diagonal(void) const
{
...
void SparseMatrix<T>::set_row(const size_t& row_index, const Vector<T>& new_row)
{
    ...
    {
        const Vector<size_t> row_occurrences_indices = rows_indices.arrange_equal_to_indices(row_index);

        rows_indices = rows_indices.remove_elements(row_occurrences_indices);
        columns_indices = columns_indices.remove_elements(row_occurrences_indices);
        matrix_values = matrix_values.remove_elements(row_occurrences_indices);
    }
...
void SparseMatrix<T>::set_column(const size_t& column_index, const Vector<T>& new_column)
{
    ...
    {
        const Vector<size_t> column_occurrences_indices = columns_indices.arrange_equal_to_indices(column_index);

        rows_indices = rows_indices.remove_elements(column_occurrences_indices);
        columns_indices = columns_indices.remove_elements(column_occurrences_indices);
        matrix_values = matrix_values.remove_elements(column_occurrences_indices);
    }
...
void SparseMatrix<T>::set_diagonal(const T& new_diagonal)
{
    ...
    if(previous_nonzero_values_number != 0)
    {
        ...
        rows_indices = rows_indices.remove_elements(diagonal_indices);
        columns_indices = columns_indices.remove_elements(diagonal_indices);
        matrix_values = matrix_values.remove_elements(diagonal_indices);
    }
...
template <class T>
void SparseMatrix<T>::initialize_identity(void)
{
...
SparseMatrix<T> SparseMatrix<T>::insert_row(const size_t& position, const Vector<T>& new_row) const
{
    ...
    Vector<size_t> new_matrix_values = new_sparse_matrix.get_matrix_values();
...
SparseMatrix<T> SparseMatrix<T>::merge_matrices(const SparseMatrix<T>& other_sparse_matrix, const size_t& columns_1_index, const size_t& columns_2_index) const
{
    ...
    const Vector<T> columns_2 = other_sparse_matrix.arrange_column(columns_2_index);
    ...
    for(size_t i = 0; i < columns_1_size; i++)
    {
        ...
        for(size_t j = 0; j < columns_2_equal_indices_size; j++)
        {
            ...
            current_row = current_row.assemble(other_sparse_matrix.arrange_row(columns_2_equal_indices[j]).remove_element(columns_2_index));
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::remove_row(const size_t& row_index) const
{
#ifdef __OPENNN_DEBUG__
    ...
    const Vector<size_t> rows_equal_than_position_indices = new_rows_indices.arrange_equal_to_indices(row_index);

    new_rows_indices = new_rows_indices.remove_elements(rows_equal_than_position_indices);
    new_columns_indices = new_columns_indices.remove_elements(rows_equal_than_position_indices);
    new_matrix_values = new_matrix_values.remove_elements(rows_equal_than_position_indices);
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::remove_rows(const Vector<size_t>& rows_to_remove) const
{
    SparseMatrix<T> new_sparse_matrix(*this);

    new_sparse_matrix.set_rows_number(rows_number-rows_to_remove.size());

    Vector<size_t> new_rows_indices = new_sparse_matrix.get_rows_indices();
    Vector<size_t> new_columns_indices = new_sparse_matrix.get_columns_indices();
    Vector<size_t> new_matrix_values = new_sparse_matrix.get_matrix_values();

    const Vector<size_t> rows_equal_than_position_indices = new_rows_indices.arrange_equal_to_indices(rows_to_remove);
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::remove_rows_with_value(const T& value) const
{
    Vector<size_t> valid_indices;

    for(size_t i = 0; i < rows_number; i++)
    {
        const Vector<T> current_row = arrange_row(i);
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::remove_first_rows(const size_t& number) const
{
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::arrange_first_rows(const size_t& number) const
{
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::remove_last_rows(const size_t& number) const
{
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::arrange_last_rows(const size_t& number) const
{
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::remove_column(const size_t& column_index) const
{
#ifdef __OPENNN_DEBUG__
    ...
    const Vector<size_t> columns_equal_than_position_indices = new_columns_indices.arrange_equal_to_indices(column_index);

    new_rows_indices = new_rows_indices.remove_elements(columns_equal_than_position_indices);
    new_columns_indices = new_columns_indices.remove_elements(columns_equal_than_position_indices);
    new_matrix_values = new_matrix_values.remove_elements(columns_equal_than_position_indices);
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::remove_columns(const Vector<size_t>& columns_to_remove) const
{
    SparseMatrix<T> new_sparse_matrix(*this);

    new_sparse_matrix.set_columns_number(columns_number-columns_to_remove.size());

    Vector<size_t> new_rows_indices = new_sparse_matrix.get_rows_indices();
    Vector<size_t> new_columns_indices = new_sparse_matrix.get_columns_indices();
    Vector<size_t> new_matrix_values = new_sparse_matrix.get_matrix_values();

    const Vector<size_t> rows_equal_than_position_indices = new_rows_indices.arrange_equal_to_indices(columns_to_remove);
...
SparseMatrix<T> SparseMatrix<T>::remove_constant_rows() const
{
    ...
    for(size_t i = 0; i < rows_number; i++)
    {
        ...
        const Vector<T> row = arrange_row(i);
    ...
    return remove_rows(rows_to_remove);
}
...
SparseMatrix<T> SparseMatrix<T>::remove_constant_columns() const
{
    ...
    for(size_t i = 0; i < columns_number; i++)
    {
        ...
        const Vector<T> column = arrange_column(i);
    ...
    return remove_columns(columns_to_remove);
}
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::sort_ascending(const size_t&) const
{

}

template <class T>
SparseMatrix<T> SparseMatrix<T>::sort_descending(const size_t&) const
{
...
void SparseMatrix<T>::replace(const T& find_what, const T& replace_with)
{
    ...
    else if(find_what == T())
    {
        ...
        for(size_t i = 0; i < rows_number; i++)
        {
            const Vector<size_t> current_row_nonzero_indices = rows_indices.arrange_equal_to_indices(i);

            const Vector<size_t> current_zero_columns = sequential_indices.get_difference(columns_indices.arrange_subvector(current_row_nonzero_indices));
    ...
    {
        const Vector<size_t> equal_than_find_indices = matrix_values.arrange_equal_to_indices(find_what);

        rows_indices = rows_indices.remove_elements(equal_than_find_indices);
        columns_indices = columns_indices.remove_elements(equal_than_find_indices);
        matrix_values = matrix_values.remove_elements(equal_than_find_indices);
    }
}
...
void SparseMatrix<T>::replace_in_row(const size_t& row_index, const T& find_what, const T& replace_with)
{
    ...
    {
        const Vector<size_t> current_row_nonzero_indices = rows_indices.arrange_equal_to_indices(row_index);
    ...
    else if(replace_with == T())
    {
        ...
        const Vector<size_t> equal_than_find_indices = matrix_values.arrange_subvector(current_row_nonzero_indices).arrange_equal_to_indices(find_what);
...
void SparseMatrix<T>::replace_in_column(const size_t& column_index, const T& find_what, const T& replace_with)
{
    ...
    {
        const Vector<size_t> current_column_nonzero_indices = columns_indices.arrange_equal_to_indices(column_index);
    ...
    else if(find_what == T())
    {
        ...
        const Vector<size_t> current_zero_rows = sequential_indices.get_difference(rows_indices.arrange_subvector(current_column_nonzero_indices));
    ...
    else if(replace_with == T())
    {
        ...
        const Vector<size_t> equal_than_find_indices = matrix_values.arrange_subvector(current_column_nonzero_indices).arrange_equal_to_indices(find_what);
...
{
    return arrange_column(column_index).contains(value);
}

// Mathematical methods

template <class T>
T SparseMatrix<T>::calculate_sum(void) const
{
    return matrix_values.calculate_sum();
}

template <class T>
Vector<int> SparseMatrix<T>::calculate_rows_sum_int(void) const
{
    // Control sentence (if debug)
    ...
    if(rows_number == 0)
    {
        ...
        cout << "OpenNN Exception: SparseMatrix Template.\n"
             << "Vector<int> calculate_rows_sum_int(void) const method.\n"
    ...
    for(size_t i = 0; i < rows_number; i++)
    {
        ...
        for(size_t j = 0; j < current_row_nonzero_number; j++)
        {
            rows_sum[i] += (int)matrix_values[current_row_nonzero_indices[j]];
        }
    }
...
template <class T>
Vector<T> SparseMatrix<T>::calculate_rows_sum(void) const
{
#ifdef __OPENNN_DEBUG__
    ...
    for(size_t i = 0; i < rows_number; i++)
    {
        ...
        for(size_t j = 0; j < current_row_nonzero_number; j++)
        {
            rows_sum[i] += matrix_values[current_row_nonzero_indices[j]];
        }
    }
...
template <class T>
Vector<T> SparseMatrix<T>::calculate_columns_sum(void) const
{
#ifdef __OPENNN_DEBUG__
    ...
    for(size_t i = 0; i < columns_number; i++)
    {
        const Vector<size_t> current_column_nonzero_indices = columns_indices.arrange_equal_to_indices(i);

        const size_t current_column_nonzero_number = current_column_nonzero_indices.size();

        for(size_t j = 0; j < current_column_nonzero_number; j++)
        {
            columns_sum[i] += matrix_values[current_column_nonzero_indices[j]];
        }
    }
...
void SparseMatrix<T>::sum_row(const size_t& row_index, const Vector<T>& vector)
{
    ...
    const Vector<T> current_row = arrange_row(row_index) + vector;

    set_row(row_index, current_row);
}

template <class T>
double SparseMatrix<T>::calculate_trace(void) const
{
    // Control sentence (if debug)
...
template <class T>
Vector<double> SparseMatrix<T>::calculate_mean(void) const
{
    // Control sentence (if debug)
...
template <class T>
Vector< Vector<double> > SparseMatrix<T>::calculate_mean_standard_deviation(void) const
{
    // Control sentence (if debug)
    ...
    for(size_t i = 0; i < columns_number; i++)
    {
        const Vector<T> current_column = arrange_column(i);
...
Vector< Vector<double> > SparseMatrix<T>::calculate_mean_standard_deviation(const Vector<size_t>& row_indices, const Vector<size_t>& column_indices) const
{
    ...
    for(size_t i = 0; i < column_indices_size; i++)
    {
        ...
        column = column.arrange_subvector(row_indices);
...
template <class T>
Vector<double> SparseMatrix<T>::calculate_median(void) const
{
    // Control sentence (if debug)
...
Vector<double> SparseMatrix<T>::calculate_median(const Vector<size_t>& median_row_indices, const Vector<size_t>& median_column_indices) const
{
    ...
    for(size_t j = 0; j < column_indices_size; j++)
    {
        ...
        sorted_column = sorted_column.arrange_subvector(median_row_indices);
...
#pragma omp parallel for schedule(dynamic)
    ...
    for(int j = 0; j < (int)column_indices_size; j++)
    {
        ...
        sorted_column = sorted_column.arrange_subvector(used_rows[j]);
...
template <class T>
T SparseMatrix<T>::calculate_minimum(void) const
{
    return std::min(T(), matrix_values.calculate_minimum());
}

template <class T>
T SparseMatrix<T>::calculate_maximum(void) const
{
    return std::min(T(), matrix_values.calculate_maximum());
}

template <class T>
T SparseMatrix<T>::calculate_column_minimum(const size_t& column_index) const
{
    return arrange_column(column_index).calculate_minimum();
}

template <class T>
T SparseMatrix<T>::calculate_column_maximum(const size_t& column_index) const
{
    return arrange_column(column_index).calculate_maximum();
}

template <class T>
Vector<T> SparseMatrix<T>::calculate_means_binary(void) const
{
    if(columns_number == 2)
    {
...
template <class T>
Vector<T> SparseMatrix<T>::calculate_means_binary_column(void) const
{
    Vector<T> means(2,0.0);

    size_t count = 0;

    Vector<T> first_column = arrange_column(0);
    Vector<T> second_column = arrange_column(1);
...
template <class T>
Vector<T> SparseMatrix<T>::calculate_means_binary_columns(void) const
{
    ...
    const Vector<T> last_column = arrange_column(columns_number-1);
...
template <class T>
Vector< Vector<T> > SparseMatrix<T>::calculate_minimum_maximum(void) const
{
    ...
    for(size_t j = 0; j < columns_number; j++)
    {
        const Vector<size_t> current_column_nonzero_indices = columns_indices.arrange_equal_to_indices(j);

        const Vector<T> current_column_nonzero_values = matrix_values.arrange_subvector(current_column_nonzero_indices);

        Vector<T> current_minimum_maximum = current_column_nonzero_values.calculate_minimum_maximum();

        minimum[j] = std::min(T(),current_minimum_maximum[0]);
        maximum[j] = std::max(T(),current_minimum_maximum[1]);
    }
...
Vector< Vector<T> > SparseMatrix<T>::calculate_minimum_maximum(const Vector<size_t>& calculate_row_indices, const Vector<size_t>& calculate_column_indices) const
{
    ...
    for(size_t j = 0; j < column_indices_size; j++)
    {
        ...
        const Vector<T> current_column = arrange_column(j);
...
template <class T>
Vector< Statistics<T> > SparseMatrix<T>::calculate_statistics(void) const
{
    // Control sentence (if debug)
...
Vector< Statistics<T> > SparseMatrix<T>::calculate_statistics(const Vector<size_t>& calculate_row_indices, const Vector<size_t>& calculate_column_indices) const
{
    ...
    for(size_t i = 0; i < column_indices_size; i++)
    {
        const Vector<T> column = arrange_column(calculate_column_indices[i]);
                                                                                  ...
                                                                                  const Vector< Vector<size_t> >& missing_indices) const
{
    ...
    for(size_t i = 0; i < columns_number; i++)
    {
        ...
        column = column.arrange_subvector(calculate_row_indices);
...
Vector<T> SparseMatrix<T>::calculate_rows_means(const Vector<size_t>& calculate_row_indices) const
{
    ...
    for(size_t i = 0; i < columns_number; i++)
    {
        ...
        column = column.arrange_subvector(used_row_indices);
                                                              ...
                                                              const Vector<size_t>& calculate_columns_indices) const
{
    ...
    {
        const Vector<size_t> current_column = arrange_column(calculate_columns_indices[i]).arrange_subvector(calculate_rows_indices[i]);
...
template <class T>
SparseMatrix<double> SparseMatrix<T>::calculate_covariance_sparse_matrix(void) const
{
    ...
    if(size == 0)
    {
        ...
        buffer << "OpenNN Exception: SparseMatrix template."
               << "SparseMatrix<double> calculate_covariance_sparse_matrix(void) const method.\n"
               << "Number of columns must be greater than zero.\n";
    ...
    for(size_t i = 0; i < size; i++)
    {
        ...
        {
            second_column = arrange_column(j);
...
void SparseMatrix<T>::scale_mean_standard_deviation(const Vector< Statistics<T> >& statistics)
{
    ...
    for(size_t j = 0; j < columns_number; j++)
    {
        ...
        {
            Vector<double> current_column = arrange_column(j);
...
template <class T>
Vector< Statistics<T> > SparseMatrix<T>::scale_mean_standard_deviation(void)
{
...
void SparseMatrix<T>::scale_columns_mean_standard_deviation(const Vector< Statistics<T> >& statistics, const Vector<size_t>& scale_columns_indices)
{
    ...
    for(size_t j = 0; j < columns_indices_size; j++)
    {
        ...
        else
        {
            ...
            Vector<double> current_column = arrange_column(column_index);
...
template <class T>
Vector< Statistics<T> > SparseMatrix<T>::scale_minimum_maximum(void)
{
    const Vector< Statistics<T> > statistics = calculate_statistics();
...
template <class T>
Vector< Statistics<T> > SparseMatrix<T>::scale_logarithmic(void)
{
    const Vector< Statistics<T> > statistics = calculate_statistics();
...
template <class T>
Vector<size_t> SparseMatrix<T>::calculate_minimal_indices(void) const
{
    ...
    if(minimum != T())
    {
        const size_t minimum_index = matrix_values.arrange_equal_to_indices(minimum)[0];
    ...
    else
    {
        for(size_t i = 0; i < rows_number; i++)
        {
            const Vector<size_t> current_row = arrange_row(i);

            if(current_row.contains(minimum))
            {
                minimal_indices[0] = i;
                minimal_indices[1] = current_row.arrange_equal_to_indices(minimum);
...
template <class T>
Vector<size_t> SparseMatrix<T>::calculate_maximal_indices(void) const
{
    ...
    if(maximum != T())
    {
        const size_t maximum_index = matrix_values.arrange_equal_to_indices(maximum)[0];
    ...
    else
    {
        for(size_t i = 0; i < rows_number; i++)
        {
            ...
            {
                maximal_indices[0] = i;
                maximal_indices[1] = current_row.arrange_equal_to_indices(maximum);
...
template <class T>
Vector< Vector<size_t> > SparseMatrix<T>::calculate_minimal_maximal_indices(void) const
{
...
double SparseMatrix<T>::calculate_sum_squared_error(const SparseMatrix<double>& other_sparse_matrix) const
{
    ...
    {
        const Vector<T> this_current_column = this->arrange_column(i);
        const Vector<T> other_current_column = other_sparse_matrix.arrange_column(i);
...
double SparseMatrix<T>::calculate_sum_squared_error(const Matrix<T>& other_matrix) const
{
    ...
    {
        const Vector<T> this_current_column = this->arrange_column(i);
        const Vector<T> other_current_column = other_matrix.arrange_column(i);
...
template <class T>
Vector<double> SparseMatrix<T>::calculate_rows_norm(void) const
{
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::calculate_absolute_value(void) const
{
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::calculate_transpose(void) const
{
...
template <class T>
T SparseMatrix<T>::calculate_determinant(void) const
{
    // Control sentence (if debug)
    ...
    else
    {
        ...
        for(size_t column_index = 0; column_index < columns_number; column_index++)
        {
            ...
            Vector<size_t> sub_matrix_values = matrix_values;

            const Vector<size_t> current_column_indices = columns_indices.arrange_equal_to_indices(column_index);

            sub_rows_indices = sub_rows_indices.remove_elements(current_column_indices);
            sub_columns_indices = sub_columns_indices.remove_elements(current_column_indices);
            sub_matrix_values = sub_matrix_values.remove_elements(current_column_indices);
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::calculate_cofactor(void) const
{
    // Control sentence (if debug)
    ...
    if(empty())
    {
        ...
        buffer << "OpenNN Exception: SparseMatrix Template.\n"
               << "SparseMatrix<T> calculate_cofactor(void) const method.\n"
               << "Sparse matrix is empty.\n";
    ...
    const Vector<size_t> sequential_row_indices(0,1,rows_number);
    const Vector<size_t> sequential_column_indices(0,1,columns_number);

    for(size_t i = 0; i < rows_number; i++)
    {
        for(size_t j = 0; j < columns_number; j++)
        {
            c = this->arrange_sub_sparse_matrix(sequential_row_indices.remove_element(i), sequential_column_indices.remove_element(j));
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::calculate_inverse(void) const
{
    // Control sentence (if debug)
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::calculate_LU_inverse(void) const /// @todo
{
    // Control sentence (if debug)
...
SparseMatrix<T> SparseMatrix<T>::operator + (const T& scalar) const
{
    ...
    {
        const Vector<T> current_row = arrange_row(i) + scalar;
...
SparseMatrix<T> SparseMatrix<T>::operator + (const Vector<T>& vector) const
{
    ...
    for(size_t i = 0; i < columns_number; i++)
    {
        const Vector<T> current_column = arrange_column(i) + vector;
...
SparseMatrix<T> SparseMatrix<T>::operator + (const SparseMatrix<T>& other_sparse_matrix) const
{
    ...
    for(size_t i = 0; i < columns_number; i++)
    {
        const Vector<T> current_column = arrange_column(i) + other_sparse_matrix.arrange_column(i);
...
SparseMatrix<T> SparseMatrix<T>::operator + (const Matrix<T>& other_matrix) const
{
    ...
    for(size_t i = 0; i < columns_number; i++)
    {
        const Vector<T> current_column = arrange_column(i) + other_matrix.arrange_column(i);
...
SparseMatrix<T> SparseMatrix<T>::operator / (const Vector<T>& vector) const
{
    ...
    for(size_t i = 0; i < rows_number; i++)
    {
        const Vector<T> current_row = arrange_row(i) / vector;
...
void SparseMatrix<T>::operator += (const T& scalar)
{
    for(size_t i = 0; i < columns_number; i++)
    {
        const Vector<T> current_column = arrange_column(i) + scalar;
...
template <class T>
Vector<double> SparseMatrix<T>::dot(const Vector<double>& vector) const /// @todo
{
    // Control sentence (if debug)
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::calculate_eigenvalues(void) const /// @todo
{
    // Control sentence (if debug)
    ...
    if((*this).get_columns_number() == 0)
    {
        ...
        buffer << "OpenNN Exception: SparseMatrix Template.\n"
               << "SparseMatrix<T> calculate_eigen_values(void) const method.\n"
               << "Number of columns must be greater than zero.\n";
    ...
    SparseMatrix<T> eigenvalues(rows_number, 1);
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::calculate_eigenvectors(void) const /// @todo
{
    // Control sentence (if debug)
    ...
    SparseMatrix<T> eigenvectors(rows_number, rows_number);
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::direct(const SparseMatrix<T>& other_sparse_matrix) const /// @todo
{
    const size_t other_rows_number = other_sparse_matrix.get_rows_number();
    const size_t other_columns_number = other_sparse_matrix.get_columns_number();
        ...
        for(size_t j = 0; j < columns_number; j++)
        {
            for(size_t k = 0; k < other_rows_number; k++)
            {
                for(size_t l = 0; l < other_columns_number; l++)
                {
                    ...
                    direct(alpha,beta) = (*this)(i,j)*other_sparse_matrix(k,l);
                }
            }
        }
    }
...
template <class T>
SparseMatrix<T> SparseMatrix<T>::direct(const Matrix<T>&) const /// @todo
{

}

template <class T>
bool SparseMatrix<T>::empty(void) const
{
...
template <class T>
bool SparseMatrix<T>::is_square(void) const
{
    if(rows_number == columns_number)
    {
        return(true);
...
template <class T>
bool SparseMatrix<T>::is_symmetric(void) const
{
    // Control sentence (if debug)
...
template <class T>
bool SparseMatrix<T>::is_antisymmetric(void) const
{
    // Control sentence (if debug)
...
template <class T>
bool SparseMatrix<T>::is_diagonal(void) const
{
    // Control sentence (if debug)
...
template <class T>
bool SparseMatrix<T>::is_scalar(void) const
{
    // Control sentence (if debug)
...
template <class T>
bool SparseMatrix<T>::is_identity(void) const
{
    // Control sentence (if debug)
...
template <class T>
bool SparseMatrix<T>::is_binary(void) const
{
...
bool SparseMatrix<T>::is_column_binary(const size_t& column_index) const
{
    ...
    return (matrix_values.arrange_subvector(column_nonzero_indices) == 1);
}
...
bool SparseMatrix<T>::is_column_constant(const size_t& column_index) const
{
    ...
    return (matrix_values.arrange_subvector(column_nonzero_indices).is_constant());
}
...
template <class T>
void SparseMatrix<T>::convert_association(void)
{
...
template <class T>
KMeansResults<T> SparseMatrix<T>::calculate_k_means(const size_t&) const /// @todo
{
...
template <class T>
Vector<T> SparseMatrix<T>::calculate_multiple_linear_regression_parameters(const Vector<T>&) const /// @todo
{
...
template <class T>
void SparseMatrix<T>::load_strings(const string&, const char&) /// @todo
{
...
template <class T>
SparseMatrix<size_t> SparseMatrix<T>::to_size_t_SparseMatrix(void) const
{
    SparseMatrix<size_t> size_t_sparse_matrix(rows_number, columns_number);

    const size_t nonzero_elements_number = matrix_values.size();

    Vector<size_t> new_matrix_values(nonzero_elements_number);
...
template <class T>
SparseMatrix<double> SparseMatrix<T>::to_double_SparseMatrix(void) const
{
...
template <class T>
Matrix<T> SparseMatrix<T>::to_matrix(void) const
{
    Matrix<T> dense_matrix(rows_number,columns_number);

    for(size_t i = 0; i < columns_number; i++)
    {
        const Vector<T> current_column = arrange_column(i);

        dense_matrix.set_column(i, current_column);
    }
...
template <class T>
Vector< Vector<T> > SparseMatrix<T>::to_vector_of_vectors(void) const
{
    Vector< Vector<T> > vector_of_vectors(columns_number);

    for(size_t i = 0; i < columns_number; i++)
    {
        vector_of_vectors[i] = arrange_column(i);
    }
...
template <class T>
Vector< Vector<size_t> > SparseMatrix<T>::to_CSR(Vector<T>& new_matrix_values) const /// @todo
{
    const size_t nonzero_elements_number = matrix_values.size();

    Vector< Vector<size_t> > CSR_indices(2);

    CSR_indices[0].set(columns_number+1, 0);
    CSR_indices[1].set(nonzero_elements_number);
    ...
    for(size_t i = 0; i < columns_number; i++)
    {
        ...
        if(current_column.count_equal_to(T()) == rows_number)
        {
            continue;
        }
        ...
        for(size_t j = 0; j < rows_number; j++)
        {
            if(current_column[j] != T())
            {
                for(size_t k = i+1; k <= columns_number; k++) /// ???
                {
                    CSR_indices[0][k]++;
                }

                CSR_indices[1][index] = j;
                new_matrix_values[index] = current_column[j];
...
template <class T>
void SparseMatrix<T>::print_preview(void) const
{
