...
// string write_loss_calculation_method(void) const method

/// Return a string with the loss calculation method of this inputs selection algorithm.

string InputsSelectionAlgorithm::write_loss_calculation_method(void) const
{
...
Matrix<double> InputsSelectionAlgorithm::calculate_logistic_correlations(void) const
{
    ...
    {
        const Vector<double> inputs = data_set_pointer->get_variable(input_indices[i],used_instances_indices);

        for(size_t j = 0; j < targets_number; j++)
        {
            const Vector<double> targets = data_set_pointer->get_variable(target_indices[j],used_instances_indices);
            ...
            Matrix<double> data(used_instances_number, 2);
            ...
            loss_index.get_weighted_squared_error_pointer()->set_normalization_coefficient();
            ...
            if(used_instances_number < 1000000)
            {
                ...
                levenberg_marquardt_algorithm->set_minimum_loss_increase(1.0e-15);

                levenberg_marquardt_algorithm->set_maximum_iterations_number(100);

                levenberg_marquardt_algorithm->set_display_period(100);
            }
            else
            {
                training_strategy.set_main_type(TrainingStrategy::QUASI_NEWTON_METHOD);
                ...
                quasi_Newton_method->set_minimum_loss_increase(1.0e-15);

                quasi_Newton_method->set_maximum_iterations_number(100);

                quasi_Newton_method->set_display_period(100);
            }
            ...
            if(display)
            {
                cout << "Calculating correlation: Input " << i+1 << "; Target " << j+1 << endl;
                cout << "Correlation value = " << abs(correlations(i,j)) << endl;
            }
...
Vector<double> InputsSelectionAlgorithm::calculate_final_correlations(void) const
{
    ...
    if(approximation)
    {
        correlations = data_set->calculate_input_target_correlations();
    }
    else
    {
...
void InputsSelectionAlgorithm::set_neural_inputs(const Vector<bool>& inputs)
{
    ...
    const size_t current_inputs_number = inputs.count_equal_to(true);
...
// string write_stopping_condition(void) const method

/// Return a string with the stopping condition of the InputsSelectionResults

string InputsSelectionAlgorithm::InputsSelectionResults::write_stopping_condition(void) const
{
...
string InputsSelectionAlgorithm::InputsSelectionResults::object_to_string(void) const
{
   ostringstream buffer;
