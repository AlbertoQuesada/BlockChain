...
const size_t& GrowingInputs::get_minimum_inputs_number(void) const
{
...
GrowingInputs::GrowingInputsResults* GrowingInputs::perform_inputs_selection(void)
{
    ...
    if(display)
    {
        cout << "Performing growing inputs selection..." << endl;
    ...
    if(final_correlations[index] >= 0.9999*targets_number)
    {
        ...
        if(display)
        {
            cout << "Maximal correlation(" << final_correlations[index] << ") is nearly 1. \n"
    ...
    else
    {
        ...
        for(size_t i = 0; i < final_correlations.size(); i++)
        {
            if(final_correlations[i] >= maximum_correlation*targets_number)
            {
                ...
                if(display)
                {
                    cout << "Added input "<< variables->arrange_names()[original_index] << endl;
                }

                if(current_inputs.count_equal_to(true) >= maximum_inputs_number)
                {
                    end = true;
    ...
    while(!end)
    {
        ...
        if(!flag_input)
        {
            ...
            if(final_correlations[index] <= minimum_correlation*targets_number)
            {
                ...
                if(current_inputs.count_equal_to(true) == 0)
                {
                    original_index = get_input_index(original_uses, index);
        ...
        else if(current_inputs.count_equal_to(true) >= maximum_inputs_number)
        {
            end = true;
        ...
        else if(current_inputs.count_equal_to(true) == current_inputs.size())
        {
            end = true;
        ...
        if(display)
        {
            ...
            if(!flag_input)
            {
                cout << "Add input: " << variables->arrange_names()[original_index] << endl;
            }
...
Matrix<string> GrowingInputs::to_string_matrix(void) const
{
    ostringstream buffer;
