...
public:
  ...
  Vector<T> arrange_not_equal_to(const T&) const;
  Vector<T> arrange_positive() const;

  Vector<size_t> arrange_equal_to_indices(const T &) const;
  Vector<size_t> arrange_equal_to_indices(const Vector<T>&) const;

  Vector<size_t> arrange_not_equal_to_indices(const T &) const;
  Vector<size_t> arrange_not_equal_to_indices(const Vector<T> &) const;

  Vector<size_t> arrange_less_than_indices(const T &) const;
  Vector<size_t> arrange_greater_than_indices(const T &) const;

  Vector<T> arrange_between(const T&, const T&) const;

  Vector<size_t> arrange_contains_indices(const std::string&) const;
  ...
  Vector<size_t> sort_ascending(void) const;

  Vector<size_t> sort_descending(void) const;
...
//  void filter(const T&, const T&);
  ...
  Vector<size_t> calculate_Tukey_outliers_indices(const double& = 1.5) const;
  ...
  Vector<T> arrange_subvector(const size_t&, const size_t&) const;
  ...
  Vector<T> arrange_integers(const size_t&) const;
  Vector<T> arrange_integers_missing_values(const Vector<size_t>&,
                                            const size_t&) const;
  ...
  Vector<T> remove_elements(const Vector<size_t> &) const;
  ...
  Vector<T> arrange_unique(void) const;
  Vector<T> arrange_unique_unsorted(void) const;
  Vector<size_t> arrange_unique_indices(void) const;
...
template <class T>
T Vector<T>::get_last_value() const
{
    const size_t this_size = this->size();

    const T last_value = (*this)[this_size-1];
...
// bool is_integer(void) const method
...
template <class T> size_t Vector<T>::perform_distribution_distance_analysis_missing_values(const Vector<size_t>& missing_indices) const
{
    ...
    const Vector<T> used_values = this->arrange_subvector(used_indices);
...
Matrix<T> Vector<T>::arrange_binary_matrix(const char& separator) const
{
    ...
    const Vector<T> unique_mixes = arrange_unique();
    ...
    for(int i = 0; i < unique_mixes.size(); i++)
    {
        ...
        unique_items = unique_items.assemble(items[i]).arrange_unique();
    }
...
template <class T>
Vector<T> Vector<T>::arrange_not_equal_to(const T& value) const
{
    const size_t this_size = this->size();
...
template <class T>
Vector<T> Vector<T>::arrange_positive(void) const
{
    const size_t this_size = this->size();
...
template <class T>
Vector<size_t> Vector<T>::arrange_equal_to_indices(const T &value) const {
  const size_t this_size = this->size();
...
template <class T>
Vector<size_t> Vector<T>::arrange_equal_to_indices(const Vector<T>&values) const
{
...
template <class T>
Vector<size_t> Vector<T>::arrange_not_equal_to_indices(const T &value) const {
...
template <class T>
Vector<size_t> Vector<T>::arrange_not_equal_to_indices(const Vector<T> &values) const
{
  const size_t this_size = this->size();

  Vector<size_t> not_equal_to_indices;

  for (size_t i = 0; i < this_size; i++)
  {
    if(!values.contains((*this)[i]))
    {
      not_equal_to_indices.push_back(i);
    }
  }
...
template <class T>
Vector<size_t> Vector<T>::arrange_less_than_indices(const T &value) const {
...
template <class T>
Vector<size_t> Vector<T>::arrange_greater_than_indices(const T &value) const
{
  const size_t this_size = this->size();
...
template <class T>
Vector<T> Vector<T>::arrange_between(const T &minimum, const T &maximum) const
{
    const size_t this_size = this->size();
...
template <class T>
Vector<size_t> Vector<T>::arrange_contains_indices(const std::string& find_what) const
{
    const size_t this_size = this->size();
...
double Vector<T>::calculate_error(const Vector<T>& other_vector) const
{
    ...
    error = error.arrange_not_equal_to(0);
...
Histogram<T> Vector<T>::calculate_histogram_integers(const size_t& bins_number) const {
    ...
    Vector<T> centers = this->arrange_integers(bins_number);
...
Histogram<T> Vector<T>::calculate_histogram_integers_missing_values(const Vector<size_t>& missing_indices, const size_t& bins_number) const {
    ...
    Vector<T> centers = this->arrange_integers_missing_values(missing_indices, bins_number);
    const size_t centers_number = centers.size();
...
double Vector<T>::calculate_last_exponential_moving_average(const size_t& period) const
{
    ...
    const size_t size = exponential_moving_average.size();

    return exponential_moving_average[size-1];
}
...
double Vector<T>::calculate_linear_trend(const size_t& start, const size_t& end) const
{
    ...
    const Vector<double> dependent_variable = arrange_subvector(indices);
...
// Control sentence (if debug)
  ...
  const Vector<T> unique = arrange_unique();
...
//    quartiles[2] = (sorted_vector[this_size * 3 / 4] + sorted_vector[this_size * 3 / 4 + 1]) / 2;
    quartiles[0] = sorted_vector.arrange_subvector_first(this_size/2).calculate_median();
    quartiles[1] = sorted_vector.calculate_median();
    quartiles[2] = sorted_vector.arrange_subvector_last(this_size/2).calculate_median();
  }
  else
  {
...
Vector<double> Vector<T>::calculate_percentiles() const
{
  ...
  const Vector<size_t> sorted_vector = sort_ascending();
...
Vector<double> Vector<T>::calculate_quartiles_missing_values(const Vector<size_t> & missing_indices) const
{
    ...
    const Vector<T> values_to_remove = this->arrange_subvector(missing_indices);
...
double Vector<T>::calculate_spearman_linear_correlation(const Vector<T> &other) const
{
    ...
    const Vector<T> this_unique = (*this).arrange_unique();
    ...
    const Vector<T> other_unique = other.arrange_unique();
...
template <class T>
Vector<size_t> Vector<T>::sort_ascending(void) const
{
...
template <class T>
Vector<size_t> Vector<T>::sort_descending(void) const
{
    Vector<size_t> indices(this->size());
...
template <class T>
Vector<size_t> Vector<T>::calculate_Tukey_outliers_indices(const double& cleaning_parameter) const
{
...
template <class T>
Vector<T> Vector<T>::arrange_subvector(const size_t& first_index, const size_t& last_index) const
{
#ifdef __OPENNN_DEBUG__
  ...
  if(last_index >= this_size || first_index >= this_size) {
      ...
      buffer << "OpenNN Exception: Vector Template.\n"
             << "Vector<T> arrange_subvector(const size_t&, const size_t&) const method.\n"
...
template <class T> Vector<T> Vector<T>::arrange_integers(const size_t& maximum_integers) const {
...
template <class T> Vector<T> Vector<T>::arrange_integers_missing_values(const Vector<size_t>& missing_indices, const size_t& maximum_integers) const {
...
template <class T>
Vector<T> Vector<T>::remove_elements(const Vector<size_t> &indices) const
{
    const size_t this_size = this->size();
...
template <class T>
Vector<T> Vector<T>::arrange_unique(void) const
{
...
template <class T>
Vector<T> Vector<T>::arrange_unique_unsorted(void) const
{
    Vector<T> copy_vector(*this);
...
/// Returns a vector with the indices of the unique elements, in the order given by the arrange_unique method.
/// For instance, if the input vector is (a, b, a), the output vector is (0, 1).

template <class T>
Vector<size_t> Vector<T>::arrange_unique_indices(void) const
{
    const Vector<T> unique_values = arrange_unique();
...
/// Returns a vector with the numbers of the unique elements, in the order given by the arrange_unique method.
...
Vector<T> Vector<T>::calculate_top(const size_t& rank) const
{
    ...
    const Vector<T> top = unique_matrix.arrange_column(0).arrange_subvector_first(end);
...
Matrix<T> Vector<T>::calculate_top_matrix(const size_t& rank) const
{
    ...
    if (rank < unique_size)
    {
        unique_matrix = unique_matrix.arrange_submatrix_rows(indices);
...
template <class T> Vector<size_t> Vector<T>::to_size_t_vector(void) const {
  const size_t this_size = this->size();
...
template <class T> string Vector<T>::to_text() const
{
  ostringstream buffer;
...
Vector<T> Histogram<T>::calculate_minimal_centers(void) const {
  ...
  const Vector<size_t> minimal_indices =
      frequencies.arrange_equal_to_indices(minimum_frequency);
...
Vector<T> Histogram<T>::calculate_maximal_centers(void) const {
  ...
  const Vector<size_t> maximal_indices =
      frequencies.arrange_equal_to_indices(maximum_frequency);
