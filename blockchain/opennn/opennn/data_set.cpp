...
bool DataSet::is_binary_variable(const string& variable_name) const
{
    ...
    const Vector<size_t> variable_index = variable_names.arrange_equal_to_indices(variable_name);
...
// string write_file_type(void) const

/// Returns a string with the name of the file type.

string DataSet::write_file_type(void) const
{
    switch(file_type)
    {
        case TXT:
        ...
        default:
        {
           ...
           buffer << "OpenNN Exception: DataSet class.\n"
                  << "string write_file_type(void) const method.\n"
                  << "Unknown file type.\n";
...
// string write_first_cell(void) const

/// Returns a string with the first cell for excel files.

string DataSet::write_first_cell(void) const
{
   return first_cell;
}


// string write_last_cell(void) const

/// Returns a string with the last cell for excel files.

string DataSet::write_last_cell(void) const
{
   return last_cell;
}
...
string DataSet::write_learning_task(void) const
{
    switch(learning_task)
    {
        ...
        default:
        {
            ...
            buffer << "OpenNN Exception: DataSet class.\n"
                   << "string write_learning_task(void) const method.\n"
...
// const string& get_data_file_name(void) const method

/// Returns the name of the data file.

const string& DataSet::get_data_file_name(void) const
{
...
// string get_separator_char(void) const

/// Returns the string which will be used as separator in the data file.

char DataSet::get_separator_char(void) const
{
    switch(separator)
    {
       case Space:
       ...
       default:
       {
          ...
          buffer << "OpenNN Exception: DataSet class.\n"
                 << "string get_separator_string(void) const method.\n"
                 << "Unknown separator.\n";
...
// string write_separator(void) const

/// Returns the string which will be used as separator in the data file.

string DataSet::write_separator(void) const
{
    switch(separator)
    {
       case Space:
       ...
       default:
       {
          ...
          buffer << "OpenNN Exception: DataSet class.\n"
                 << "string write_separator(void) const method.\n"
                 << "Unknown separator.\n";
...
Matrix<double> DataSet::arrange_used_data(void) const
{
   ...
   return(data.arrange_submatrix(instances_indices, variables_indices));
}
...
// Vector<double> arrange_testing_time_column(void) const method

/// Returns a column with the testing instances and the time variable.
/// The number of rows is the number of testing instances.

Vector<double> DataSet::arrange_testing_time_column(void) const
{
   const Vector<size_t> testing_indices = instances.arrange_testing_indices();

   Matrix<double> matrix = data.arrange_submatrix_rows(testing_indices);

   const size_t time_index = variables.get_time_index();

   return(matrix.arrange_column(time_index));
}
...
Vector<double> DataSet::get_variable(const string& variable_name) const
{
    ...
    return(data.arrange_column(variable_index[0]));
}
...
Vector<double> DataSet::get_variable(const string& variable_name, const Vector<size_t>& instances_indices) const
{
    ...
    return(data.arrange_column(variable_index[0], instances_indices));
}
...
void DataSet::remove_instance(const size_t& instance_index)
{
   ...
   data.remove_row(instance_index);
...
void DataSet::remove_variable(const size_t& variable_index)
{
   ...
   data.remove_column(variable_index);
...
Vector<string> DataSet::unuse_constant_variables(void)
{
   const size_t variables_number = variables.get_variables_number();
   ...
   if(variables_number == 0)
   {
      ...
      buffer << "OpenNN Exception: DataSet class.\n"
             << "Vector<string> unuse_constant_variables(void) method.\n"
   ...
   Vector<size_t> constant_variables(variables_number, 0);

   #pragma omp parallel for schedule(dynamic)

   for(int i = 0; i < (int)variables_number; i++)
   {
      if(variables.get_use(i) ==  Variables::Input
      && data.is_column_constant(i))
      {
         variables.set_use(i, Variables::Unused);

         constant_variables[i] = 1;
      }
   }

   const size_t constant_variables_number = constant_variables.calculate_sum();
/*
   Vector<size_t> constant_variables_indices(constant_variables_number);

   size_t index = 0;

   for(size_t i = 0; i < variables_number; i++)
   {
       if(constant_variables[i] == 1)
       {
           constant_variables_indices[index] = i;
           index++;
       }
   }

   return(constant_variables_indices);
*/

   Vector<string> constant_variables_names(constant_variables_number);
   ...
   for(size_t i = 0; i < variables_number; i++)
   {
       if(constant_variables[i] == 1)
       {
           constant_variables_names[index] = variables.get_name(i);
           index++;
       }
   }

   return(constant_variables_names);
...
Vector< Histogram<double> > DataSet::calculate_data_histograms(const size_t& bins_number) const
{
   ...
   {
       column = data.arrange_column(used_variables_indices[i], used_indices[i]);
...
Matrix<double> DataSet::calculate_positives_data_statistics_matrix(void) const
{
    ...
    const Vector<size_t> positives_used_instances_indices = used_instances_indices.arrange_subvector(targets.arrange_equal_to_indices(1.0));
    ...
    for(size_t i = 0; i < inputs_number; i++)
    {
        ...
        const Vector<double> variable_data = data.arrange_column(variable_index, current_variable_positives_instances_index);
...
Matrix<double> DataSet::calculate_negatives_data_statistics_matrix(void) const
{
    ...
    const Vector<size_t> negatives_used_instances_indices = used_instances_indices.arrange_subvector(targets.arrange_equal_to_indices(0.0));
    ...
    for(size_t i = 0; i < inputs_number; i++)
    {
        ...
        const Vector<double> variable_data = data.arrange_column(variable_index, current_variable_negatives_instances_index);
...
#pragma omp parallel for
    ...
    for(int i = 0; i < (int)variables_number; i++)
    {
        ...
        means[i] = data.arrange_column(variable_index).calculate_mean();
    }
...
// Statistics<double> calculate_inputs_statistics(void) const method
...
Statistics<double> DataSet::calculate_input_statistics(const size_t& input_index) const
{
   ...
   return(data.arrange_column(input_index).calculate_statistics_missing_values(missing_indices));
}
...
// Vector< Vector< Vector<double> > > calculate_means_columns(void) const method

/// @todo

Vector< Vector< Vector<double> > > DataSet::calculate_means_columns(void) const
{
    const size_t inputs_number = variables.count_inputs_number();
    const size_t targets_number = variables.count_targets_number();
    const Vector<size_t> inputs_indices = variables.arrange_inputs_indices();
...
// Matrix<double> calculate_input_target_correlations(void) const method
...
Matrix<double> DataSet::calculate_input_target_correlations(void) const
{
   const size_t inputs_number = variables.count_inputs_number();
   const size_t targets_number = variables.count_targets_number();
   ...
   for(int i = 0; i < (int)inputs_number; i++)
   {
       for(int j = 0; j < (int)targets_number; j++)
       {
           ...
           const Vector<double> input_variable = data.arrange_column(input_index);

           const size_t target_index = target_indices[j];

           const Vector<double> target_variable = data.arrange_column(target_index);
           ...
           linear_correlations(i,j) = input_variable.calculate_linear_correlation_missing_values(target_variable, missing_instances);
       }
   }
...
void DataSet::print_missing_values_information(void) const
{
    const size_t variables_number = variables.get_variables_number();
...
void DataSet::print_input_target_correlations(void) const
{
    const size_t inputs_number = variables.count_inputs_number();
    const size_t targets_number = variables.count_targets_number();
...
void DataSet::print_top_input_target_correlations(const size_t& number) const
{
    ...
    for(size_t j = 0; j < targets_number; j++)
    {
        ...
        target_correlations = linear_correlations.arrange_column(j);

        top_correlations.set_column(0, inputs_name);
        top_correlations.set_column(1, target_correlations.to_string_vector());
...
Matrix<double> DataSet::calculate_variables_correlations(void) const
{
    const size_t variables_number = variables.get_variables_number();
    ...
    for(size_t i = 0; i < variables_number; i++)
    {
        ...
        for(size_t j = i; j < variables_number; j++)
        {
            ...
            correlations(i,j) = variable_i.calculate_linear_correlation_missing_values(variable_j, missing_indices_i.get_union(missing_indices_j));
        }
    }
...
void DataSet::print_variables_correlations(void) const
{
...
void DataSet::print_top_variables_correlations(const size_t& number) const
{
    ...
    for(size_t i = 0; i < variables_number; i++)
    {
        for(size_t j = i; j < variables_number; j++)
        {
            ...
            top_correlations(row, 2) = std::to_string(variables_correlations(i,j));
...
Matrix<double> DataSet::calculate_multiple_linear_correlations(const Vector<size_t> & nominal_variables) const
{
    ...
    for(size_t i = 0; i < inputs_number; i++)
    {
        for(size_t j = 0; j < targets_number; j++)
        {
            ...
            input_variables = data.arrange_submatrix_columns(input_indices);
            ...
            {
                multiple_linear_correlations(i,j) = input_variables.to_vector().calculate_linear_correlation(target_variable);
            }
            else
            {
                multiple_linear_correlations(i,j) = input_variables.calculate_multiple_linear_correlation(target_variable);
            }
        }
    }
...
Matrix<double> DataSet::perform_principal_components_analysis(const double& minimum_explained_variance)
{
    ...
    const Vector<size_t> sorted_principal_components_indices = explained_variance.sort_descending();
...
// void remove_input_data_mean(void) method

/// Subtracts off the mean to every of the input variables.

void DataSet::remove_input_data_mean(void)
{
...
// Vector<string> calculate_default_scaling_methods(void) const method

/// Returns a vector of strings containing the scaling method that best fits each
/// of the input variables.

Vector<string> DataSet::calculate_default_scaling_methods(void) const
{
...
//#pragma omp parallel for private(current_distribution)
    ...
    {
//        current_distribution = data.arrange_column(used_inputs_indices[i]).perform_distribution_distance_analysis_missing_values(missing_indices[i]);
        current_distribution = data.arrange_column(used_inputs_indices[i]).perform_distribution_distance_analysis();
...
{
    Vector<double> column = data.arrange_column(input_index);
...
void DataSet::scale_targets_logarithmic(const Vector< Statistics<double> >& targets_statistics)
{
    ...
    if(data.empty())
    {
       ...
       buffer << "OpenNN Exception: DataSet class.\n"
              << "Vector< Statistics<double> > scale_targets_logarithmic(void) method.\n"
              << "Data file is not loaded.\n";
...
Vector< Statistics<double> > DataSet::scale_targets_logarithmic(void)
{
   const Vector< Statistics<double> > targets_statistics = calculate_targets_statistics();
...
string DataSet::object_to_string(void) const
{
   ostringstream buffer;
...
// Vector<string> read_header_line(void) const method

/// Returns the name of the columns in the data set as a list of strings.

Vector<string> DataSet::read_header_line(void) const
{
...
// Vector< Vector<string> > set_from_data_file(void) method
...
Vector< Vector<string> > DataSet::set_from_data_file(void)
{
...
Vector<size_t> DataSet::balance_multiple_targets_distribution(void)
{
    ...
    while(!target_class_differences.is_in(0, 0))
    {
        ...
        unbalanced_instances_indices = total_frequencies.sort_descending(0).arrange_column(1).arrange_subvector_first(unbalanced_instances_number);
...
//    #pragma omp parallel for private(i, instance_index, instance, instance_value, instance_bin, instance_frequencies) shared(count_instances, total_instances_frequencies, maximal_frequency_target_index)
    ...
    {
        most_populated_instances = total_instances_frequencies.sort_descending(0).arrange_column(1).arrange_subvector_first(maximum_frequency);
    }
    else
    {
        most_populated_instances = total_instances_frequencies.sort_descending(0).arrange_column(1).arrange_subvector_first(instances_to_unuse);
    }
...
Matrix<size_t> DataSet::calculate_k_nearest_neighbors(const Matrix<double>& distances, const size_t& nearest_neighbours_number) const
{
    const size_t instances_number = instances.count_used_instances_number();
    ...
    {
        const Vector<double> instance_distances = distances.arrange_row(i);

        const Vector<size_t> minimal_distances_indices = instance_distances.calculate_minimal_indices(nearest_neighbours_number + 1);
...
Vector<double> DataSet::calculate_k_distances(const Matrix<double>& distances, const size_t& nearest_neighbours_number) const
{
    ...
    for(int i = 0; i < instances_number; i++)
    {
        const size_t maximal_index = nearest_neighbors(i, nearest_neighbours_number - 1);
...
Vector<double> DataSet::calculate_reachability_density(const Matrix<double>& distances, const size_t& nearest_neighbours_number) const
{
   ...
   const Matrix<size_t> nearest_neighbors_indices = calculate_k_nearest_neighbors(distances, nearest_neighbours_number);
...
Vector<size_t> DataSet::calculate_Tukey_outliers(const size_t& variable_index, const double& cleaning_parameter) const
{
    ...
    const Vector<double> box_plot = data.arrange_column(variable_index).calculate_box_plot();
...
#pragma omp parallel for private(variable_index) schedule(dynamic)
    ...
    for(int i = 0; i < (int)variables_number; i++)
    {
        ...
        box_plots[i] = data.arrange_column(variable_index).calculate_box_plot();
    }
...
//    }
    ...
    for(size_t i = 0; i < maximum_lags_number; i++)
    {
        ...
        {
            autocorrelation.set_row(j, data.arrange_column(j).calculate_autocorrelation(maximum_lags_number));
        }
    }
...
// Matrix< Vector<double> > calculate_cross_correlation(const size_t&) method

/// Calculates the cross-correlation between all the variables in the data set.

Matrix< Vector<double> > DataSet::calculate_cross_correlation(const size_t& lags_number) const
{
    const size_t variables_number = variables.get_variables_number();
    ...
    for(size_t i = 0; i < variables_number; i++)
    {
        actual_column = data.arrange_column(i);

        for(size_t j = 0; j < variables_number; j++)
        {
            cross_correlation(i , j) = actual_column.calculate_cross_correlation(data.arrange_column(j), lags_number);
        }
    }
...
// Matrix<double> calculate_lag_plot(void) const method

/// @todo

Matrix<double> DataSet::calculate_lag_plot(void) const
{
    const size_t instances_number = instances.count_used_instances_number();
    ...
    lag_plot = data.arrange_submatrix_columns(columns_indices);
...
Vector< LinearRegressionParameters<double> > DataSet::perform_trends_transformation(void)
{
    const Vector<size_t> used_instances_indices = instances.arrange_used_indices();
    ...
    const size_t used_variables_number = variables.count_used_variables_number();
    const Vector<size_t> used_variables_indices = variables.arrange_used_indices();
    const size_t time_variable_index = variables.get_time_index();

    Vector< LinearRegressionParameters<double> > trends(used_variables_number - 1);

    Vector<double> independent_variable = get_variable(time_variable_index);
    ...
    for(size_t i = 0; i < used_variables_number; i++)
    {
        ...
        if(used_variable_index != time_variable_index)
        {
            ...
            independent_variable = get_variable(time_variable_index, current_used_instances);
    ...
    data.remove_trends_missing_values(trends, time_variable_index, missing_indices);
...
Vector< LinearRegressionParameters<double> > DataSet::perform_inputs_trends_transformation(void)
{
    const Vector<size_t> used_instances_indices = instances.arrange_used_indices();
    const size_t used_instances_number = used_instances_indices.size();
    ...
    data.remove_inputs_trends_missing_values(inputs_trends, time_variable_index, inputs_indices, missing_indices);
...
Vector< LinearRegressionParameters<double> > DataSet::perform_outputs_trends_transformation(void)
{
    const Vector<size_t> used_instances_indices = instances.arrange_used_indices();
    const size_t used_instances_number = used_instances_indices.size();
    ...
    data.remove_outputs_trends_missing_values(outputs_trends, time_variable_index, targets_indices, missing_indices);
...
Vector<size_t> DataSet::filter_variable(const std::string& variable_name, const double& minimum, const double& maximum)
{
...
// void impute_missing_values_unuse(void) method

/// Sets all the instances with missing values to "Unused".

void DataSet::impute_missing_values_unuse(void)
{
...
// void impute_missing_values_mean(void) method

/// Substitutes all the missing values by the mean of the corresponding variable.

void DataSet::impute_missing_values_mean(void)
{
    const Vector< Vector<size_t> > missing_indices = missing_values.arrange_missing_indices();
...
// void impute_missing_values_time_series(void) method

/// @todo

void DataSet::impute_missing_values_time_series_mean(void)
{
    const Vector< Vector<size_t> > missing_indices = missing_values.arrange_missing_indices();
...
// void impute_missing_values_time_series_regression(void)

/// @todo

void DataSet::impute_missing_values_time_series_regression(void)
{
    const Vector< Vector<size_t> > missing_indices = missing_values.arrange_missing_indices();
...
// void impute_missing_values_median(void) method

/// Substitutes all the missing values by the median of the corresponding variable.

void DataSet::impute_missing_values_median(void)
{
    const Vector< Vector<size_t> > missing_indices = missing_values.arrange_missing_indices();
