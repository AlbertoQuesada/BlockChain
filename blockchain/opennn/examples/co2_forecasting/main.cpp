...
/*                                                                                                              */
/*   C O 2  F O R E C A S T I N G   A P P L I C A T I O N                                                       */
/*                                                                                                              */
/*   Patricia Garcia                                                                                            */
...
int main(void)
{
    try
    {
        std::cout << "OpenNN. CO2 Forecasting Application." << std::endl;
        ...
        ds.set_data_file_name("../data/CO2_univariate.csv");
        ds.set_header_line(true);
        ds.set_separator("Comma");
        ds.set_missing_values_label("NA");
        ...
        //ds.scrub_missing_values_time_series_mean();


        std::cout << "Converting to time series" << std::endl;

        ds.set_lags_number(3);
        ds.set_steps_ahead_number(1);
        ds.convert_time_series();


//        std::cout << "Data autocorrelation" << std::endl;
//        Matrix<double> data_autocorrelation = ds.calculate_autocorrelation(10);
//        data_autocorrelation.save("../data/data_autocorrelation.dat");
        ...
        const size_t hidden_perceptrons_number = 7;
        ...
        quasi_Newton_method_pointer->set_maximum_time(2500);
        quasi_Newton_method_pointer->set_display_period(5);
        quasi_Newton_method_pointer->set_minimum_loss_increase(1.0e-6);
        quasi_Newton_method_pointer->set_reserve_elapsed_time_history(true);
...
//        model_selection_results.save("../data/model_selection_results.dat");
        ...
        // Testing analysis (forecasting)

        std::cout << "Testing forecasting" << std::endl;

        TestingAnalysis testing_analysis_forecasting(&neural_network, &ds_copy);
        TestingAnalysis::LinearRegressionResults forecasting_linear_regression_results = testing_analysis_forecasting.perform_forecasting_linear_regression_analysis();
        Vector< Vector<double> > forecasting_error_autocorrelation = testing_analysis_forecasting.calculate_forecasting_error_autocorrelation();
        Vector< Vector<double> > forecasting_error_crosscorrelation = testing_analysis_forecasting.calculate_forecasting_input_error_cross_correlation();
        Vector< Matrix<double> > forecasting_error_data = testing_analysis_forecasting.calculate_forecasting_error_data();
        Vector< Vector< Statistics<double> > > forecasting_error_data_statistics = testing_analysis_forecasting.calculate_forecasting_error_data_statistics();

        forecasting_linear_regression_results.save("../data/linear_forecasting_regression_analysis_results.dat");
        forecasting_error_autocorrelation.save("../data/error_forecasting_autocorrelation.dat");
        forecasting_error_crosscorrelation.save("../data/error_forecasting_crosscorrelation.dat");
        forecasting_error_data.save("../data/forecasting_error_data.dat");
        forecasting_error_data_statistics.save("../data/forecasting_error_data_statistics.dat");
